package chap16_collectionFrameWork;

import java.util.ArrayList;
import java.util.HashMap;
import java.util.Iterator;
import java.util.List;
import java.util.Map;
import java.util.Map.Entry;
import java.util.Set;

import chap06_class.car.Car;

public class _08_hashMap {

	public static void main(String[] args) {
		// TODO Auto-generated method stub
		
		//Map을 HashMap을 통해 carMap라는 객체를 생성
		Map<String, String> carMap =
				new HashMap<String, String>();
		
		//key, value의 쌍 저장 => object.put(키, 값)
		carMap.put("company", "현대");
		carMap.put("model", "소나타");
		carMap.put("color", "검정색");
		carMap.put("price", "4000");
		
		System.out.println(carMap);
		
		//Map에 어떤 데이터가 들어있는지 확인할 때
		//Entry의 Set을 생성하고 Iterator를 꺼내서
		//Key, Value를 하나씩 확인한다.
		
		//entrySet()은 carMap 객체 내에 있는 모든 키-값 쌍을 Set 타입으로 반환합니다
		//이를 변수 carSet에 할당하여
		//carSet 변수는 carMap 객체 내에 있는 모든 키-값 쌍(Entry)을 원소로 갖는
		//Set 객체가 됩니다.
		//대신 순서는 보장하지 않음.
		Set<Entry<String, String>> carSet = 
						carMap.entrySet();
		System.out.println(carMap);
		System.out.println(carMap.get("company"));
		
		Iterator<Entry<String, String>> it = 
						carSet.iterator();
		
		//hasNext는 다음요소가 있으면 트루, 없으면 폴스.
		while(it.hasNext()) {
			Entry<String, String> ent = 
									it.next(); //다음요소를 반환한다.
			System.out.println(ent.getKey());
			String key = ent.getKey();
			if(key.equals("company")) {
				ent.setValue("기아");
			}
			System.out.println(ent.getValue());
		}
		
		//value의 형태를 Object로 지정하면
		//어떤 형태의 데이터도 받을 수 있다.
		Map<String, Object> objMap =
					new HashMap<String, Object>();
		
		List<Car> carList = 
						new ArrayList<Car>();
		
		for(int i = 0; i < 5; i++) {
			Car car = 
					new Car("현대", "소나타20" + (10 + i),
							"검정", 3000);
			carList.add(car);
		}
		
		objMap.put("carList", carList);
		/*
		 * list<objMap> : [
		 * 					{
		 * 				carList : [
		 * 							car : {
		 * 
		 * 
		 * 
		 * 								   }
		 * 							car : {
		 * 
		 * 
		 * 
		 * 
		 * 								  }		
		 * 
		 * 
		 * 
		 * 
		 * 
		 * 							]
		 * 
		 * 
		 * 
		 * 
		 * 
		 * 
		 * 
		 * 
		 * 
		 * 			 },
		 * 			 {
		 * 
		 * 
		 * 
		 * 
		 * 
		 * 
		 * 
		 * 
		 * 
		 * 
		 * 			},
		 * 			]
		 * 
		 * 
		 * 
		 * */
//		List<Map<String, String>>
		
		
		
		
		
		
		
		
	}

}
